      ...
      <div className="relative z-10 mx-auto max-w-[1400px]">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {CASE_STUDIES.map((caseStudy) => (
            <article
              key={caseStudy.title}
              className="group flex flex-col justify-between rounded-2xl border border-slate-800/80 bg-[#0b111d] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-slate-700 hover:shadow-2xl md:p-8"
            >
              <div>
                <h2 className="mb-4 text-xl font-bold leading-snug text-white transition-colors duration-300 group-hover:text-[#3F8BF9]">
                  {caseStudy.title}
                </h2>
                <p className="text-sm font-light leading-relaxed text-slate-400 md:text-base">{caseStudy.summary}</p>
              </div>
            </article>
          ))}
        </div>
